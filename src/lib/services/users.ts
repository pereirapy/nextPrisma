...
export const signIn = async (
  user: Record<'email' | 'password', string> | undefined,
) => {
  try {
    ...
    if (existingUserEmail) {
      ...
      if(!existingUserPassword || existingUserPassword.length > 1) return { error: 'user_not_found!' };
      const firstUser = existingUserPassword[0];
      // Compare hash of passwords.
      if (password!== firstUser.password)
...
export const signUp = async (user: User) => {
  try {
    ...
    if (existingUser) return { error: 'email_already_taken' };

    // Check if passwords are empty.
    if (password === '')
      return { error: 'passwords_empty' };

    // Create user in DB.
    const newUser = await prisma.taddr01.create({
      data: { creatorName: name, address: email, addrtype: 'E', lang: 'EN' },
    });
    
    const tcoach02Data = await prisma.tcoach02.create({
      data: { addressId: newUser.addressId, name, password, status: 'Active'},
    });
